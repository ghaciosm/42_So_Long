...
void	collectible(t_data	*game)
{
	...
	while (i < game -> y)
    {
        ...
        while (game -> a[i][j] != '\0')
        {
            if (game -> a[i][j] == 'C')
            {		
				mlx_put_image_to_window(game -> mlx, game -> win, game -> images->coin[game->a_c], j*64 + 17, i*64 + 17);	
            }
            j++;
        }
        i++;
...
void    image_file(t_data *game)
{
	...
	game -> image5 = mlx_xpm_file_to_image(game -> mlx, "5.xpm", &(game -> width), & (game -> height));
	chr_create(game);
	game->door -> door2 = mlx_xpm_file_to_image(game -> mlx, "door2.xpm", &(game -> width), & (game -> height));
	game->door -> door3 = mlx_xpm_file_to_image(game -> mlx, "door3.xpm", &(game -> width), & (game -> height));
	game->door -> door4 = mlx_xpm_file_to_image(game -> mlx, "door4.xpm", &(game -> width), & (game -> height));
	image_coin(game);
}
